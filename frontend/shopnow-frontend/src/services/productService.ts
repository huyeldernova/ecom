...
import { productApi } from './api';
import { ApiResponse, PageResponse } from '@/types/common.types';
import { Product, ProductFilterRequest, ProductSummary } from '@/types/product.types';

export const productService = {
  // ─── Lấy danh sách sản phẩm ─────────────────────────────
  getAll: async (filter: ProductFilterRequest): Promise<PageResponse<ProductSummary>> => {
    const res = await productApi.get<ApiResponse<PageResponse<ProductSummary>>>(
      '/api/v1/products',
      { params: filter }
    );
